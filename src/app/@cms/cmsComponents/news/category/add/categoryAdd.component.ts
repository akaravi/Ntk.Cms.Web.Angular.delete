...
export class NewsCategoryAddComponent implements OnInit {
  ...
  ngOnInit() {
     this.parentId = Number.parseInt(this.activatedRoute.snapshot.paramMap.get("id"));
  ...
  dataResultCategory: ErrorExcptionResult<baseEntityCategory<number>> = new ErrorExcptionResult<baseEntityCategory<number>>();
  dataModelCategory: baseEntityCategory<number> = new baseEntityCategory<number>();
  parentId: any;
  ...
  DataAddContent() {
    this.dataModelCategory.LinkParentId=this.parentId;
    this.newsContentService.ServiceAdd<baseEntityCategory<number>>(this.dataModelCategory).subscribe(
      (next) => {
        if (next.IsSuccess) {
